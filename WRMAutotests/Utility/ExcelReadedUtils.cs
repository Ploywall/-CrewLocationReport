...
namespace WRMAutotests.Utility
{
    public class ExcelReadedUtils
    {
        ...
        public Boolean GetBooleanCellValue(int numberOFTable, int numberOfRow, int numberOfColumn)
        {
            String strValueFromExcel = GetCellValue(numberOFTable, numberOfRow, numberOfColumn);
            strValueFromExcel = strValueFromExcel.ToLower();
            if (strValueFromExcel.Equals("fault"))
                strValueFromExcel = "false";
            if (strValueFromExcel.Equals("1"))
                strValueFromExcel = "true";
            return Boolean.Parse(strValueFromExcel);
        }
